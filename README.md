...
![Version](https://img.shields.io/badge/version-0.4.0-pink.svg)
...
下载过程中脚本会自己处理各种不顺：**某个 CDN 节点连不上就自动换下一个**（B 站常把首选地址给成 P2P 节点，实测有相当比例不可用）；节点被限流就**自动降低并发**再试；节点不支持分块下载就**退回单线程**；连接卡住不动超过两分钟就重连。这些都不需要你干预。
